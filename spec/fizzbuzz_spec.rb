...
describe 'FizzBuzz' do
  ...
  context 'knows that a number is not divisible by' do
  	it '3' do
  	  expect(is_divisible_by_three?(1)).to be_falsy
  	end
  	it '5' do
  	  expect(is_divisible_by_five?(1)).to be_falsy
  	end
  	it '15' do
      expect(is_divisible_by_fifteen?(1)).to be_falsy
    end
  end

  context 'while playing game it returns' do
  	it 'the number' do
